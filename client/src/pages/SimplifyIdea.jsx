import { Edit, Lightbulb, Loader2, Sparkles } from 'lucide-react';
...
const SimplifyIdea = () => {
  const [selectedCategory, setSelectedCategory] = useState('Child');
  const [subject, setSubject] = useState('');
  const [isGenerating, setIsGenerating] = useState(false);
  const [errorGenerating, setErrorGenerating] = useState('');
  const [response, setResponse] = useState('');
  const { getToken } = useAuth();
  const { t, isRTL, currentLanguage } = useLanguage(); // Use currentLanguage from context
  const BACKEND_URL = import.meta.env.VITE_BACKEND_URL;

  const handleSimplifyIdea = async (e) => {
    e.preventDefault();
    try {
      ...
      const { data } = await axios.post(
        BACKEND_URL + '/api/ai/simplify-idea',
        {
          ...
          language: currentLanguage, // Send current language ('ar' or 'en')
        },
        {
          headers: {
  ...
  return (
    <>
      <Helmet>
        <title>{t('seoContent.simplifyIdea.title')}</title>
        <meta
          name="description"
          content={t('seoContent.simplifyIdea.description')}
        />
      </Helmet>
        ...
        <div className="flex flex-col xl:flex-row gap-6 xl:gap-8 max-w-7xl mx-auto">
          ...
          <div className="w-full xl:w-1/2">
            ...
            >
              ...
              <div className="space-y-6 mb-12">
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={`${
                      isRTL
                        ? 'أدخل بضع كلمات مفتاحية تصف الفكرة التي تبحث عن تبسيط لها...'
                        : 'Provide a few keywords that describe the idea you need me to simplify...'
                    }`}
                    required
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm font-semibold mb-3">
                    {t('simplifyIdea.category')}
                  </label>
                  <div className="flex flex-wrap gap-3">
                    {ideaCategories.map((el) => (
                      <button
                        type="button"
              ...
              <button
                onClick={handleSimplifyIdea}
                disabled={
                  subject.length < 5 || !selectedCategory || isGenerating
                }
                className="w-full flex justify-center items-center gap-2 
                px-4 py-3 mt-6 text-sm font-medium
                bg-gradient-to-r from-[#2193b0] to-[#3ca2ba]
                      ...
                      <div className="text-center space-y-4 max-w-xs">
                        ...
                        <div className="space-y-2">
                          <h3 className="text-lg font-medium">
                            {isRTL
                              ? 'ابدأ في تبسيط فكرتك'
                              : 'Start Simplifying Your Idea'}
                          </h3>
                          <p className="text-sm text-gray-400 leading-relaxed">
                            {isRTL
                              ? 'استخدم بضع كلمات مفتاحية لوصف الموضوع، اختر الفئة المستهدفة ثم انقر على "اشرح الفكرة"'
                              : 'Use keywords to describe the idea, choose a target category, then click "Explain Idea"'}
                          </p>
                        </div>
                      </div>
                    )}
